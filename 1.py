...
if __name__ == '__main__':
    ...
    yolo_net = yolo(s=CFG.S, cell_out_ch=CFG.B * 5 + 20, backbone_name=CFG.backbone, pretrain=CFG.pretrain)
    yolo_net = yolo_net.to(device)
    ...
    for images, labels, targets in dl:
        ...
        print(f"Compute time: {time.time() - start:.4f} sec")

    
    end_time = time.time()
    
    print(f"加载 {num_batches} 个 batch 的时间是: {(end_time - start_time):.6f} 秒")
